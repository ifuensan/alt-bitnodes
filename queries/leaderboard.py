"""Rankings (by country/ASN/UA) and IP groupings over the latest snapshot."""
...
def _group_ranking(rows: list[list], key_fn) -> list[dict]:
    ...
    for r in rows:
        ...
        if label is None or label == "":
            continue
        buckets[label] += 1
    out = [{"label": label, "total_nodes": count} for label, count in buckets.items()]
...
def rankings_by_country() -> list[dict]:
    ...
    grouped = _group_ranking(rows, lambda r: r[9])
    return [
        {
            "country": g["label"],
...
def rankings_by_asn() -> list[dict]:
    ...
    grouped = _group_ranking(rows, lambda r: r[13])
    return [
        {
            "asn": g["label"],
...
def rankings_by_user_agent() -> list[dict]:
    rows = _latest_snapshot_rows()
    grouped = _group_ranking(rows, lambda r: r[3])
    return [
        {
            "user_agent": g["label"],
            "total_nodes": g["total_nodes"],
