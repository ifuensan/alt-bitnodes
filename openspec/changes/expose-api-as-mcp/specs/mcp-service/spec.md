...
The server SHALL expose at minimum the following tools, each backed by `data/queries.py` and accepting validated input:
...
Each prompt SHALL produce a structured message sequence suitable for handing to an LLM, embedding relevant data fetched via the same `data/queries.py` helpers.
...
The introduction of the MCP server SHALL NOT alter the URLs, request shapes, response payloads, or status codes of the existing REST API v1 endpoints served by `app.py`. The shared `data/queries.py` refactor SHALL be transparent to REST consumers.
