...
Leaderboard and ranking endpoints (fastest nodes, by country, by ASN, by user-agent, same-IP group) backed by RTT history joined with snapshot metadata. Lets API consumers compare operators, networks, and software distributions on latency and reach.
